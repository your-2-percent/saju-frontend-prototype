...
export function equationOfTimeMinutes(date) {
    ...
    const dayOfYear = Math.floor((date.getTime() - jan1UTC) / msPerDay) + 1;
...
export function getStandardMeridian(lon) {
    // 정규화
    lon = ((lon + 540) % 360) - 180;
    // 🇰🇷/🇯🇵/🇨🇳 특례 (기존 유지)
    if (lon >= 124 && lon <= 132)
        return 135; // KST (UTC+9)
    if (lon >= 129 && lon <= 146)
        return 135; // JST (UTC+9)
    if (lon >= 73 && lon <= 135)
        return 120; // CST (UTC+8)
    // 🇺🇸 (대략 범위): 표준자오선 고정
    if (lon >= -161 && lon <= -154)
        return -150; // HST (UTC-10) ← 하와이!
    if (lon >= -170 && lon < -161)
        return -165; // Samoa/HST 경계 근처 (필요시 조정)
    if (lon >= -150 && lon < -141)
        return -135; // AKST (UTC-9)
    if (lon >= -141 && lon < -126)
        return -120; // PST  (UTC-8)
    if (lon >= -126 && lon < -111)
        return -105; // MST  (UTC-7)
    if (lon >= -111 && lon < -96)
        return -90; // CST  (UTC-6)
    if (lon >= -96 && lon < -81)
        return -75; // EST  (UTC-5)
...
export function getCorrectedDate(raw, lonInput) {
    // 벽시계 고정
    const base = new Date(raw.getFullYear(), raw.getMonth(), raw.getDate(), raw.getHours(), raw.getMinutes(), 0, 0);
    if (lonInput == null || !isFinite(lonInput))
        return base;
    // 경도 정규화 (라디안 → 도)
    let lon = lonInput;
...
export function getLocalTimes(raw, lon) {

...
function pickSL(mod) {
    ...
    return (y, m, d) => assertL2S(fn(y, m, d, false));
}
const lunar2solarStrict = pickSL(solarlunar);
/* --- 핵심: 간지/보정시 스냅샷 재계산 --- */
export function recalcGanjiSnapshot(ms) {
    const hourRule = ms.mingSikType ?? "자시";
    // 날짜 파싱
    ...
    // 경도 (모름이면 127.5)
    const lon = !ms.birthPlace || ms.birthPlace.name === "모름" || ms.birthPlace.lon === 0
        ? 127.5
        : ms.birthPlace.lon;
    // 보정

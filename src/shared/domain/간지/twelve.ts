...
export function getTwelveUnseong(baseDayStem: string, branch: string): string {
...
// 2-1. 고전 + 개화론
function shinsal_classic_gaehwa(yearBranch: string, branch: string): string {
  const mapping: Record<string, Record<string, string>> = {
...
function shinsal_classic_basic(yearBranch: string, branch: string): string {
...
// 2-3. 현대 + 개화론
function shinsal_modern_gaehwa(yearBranch: string, branch: string): string {
  const mapping: Record<string, Record<string, string>> = {
...
function shinsal_modern_basic(yearBranch: string, branch: string): string {
  const groupMapping: Record<string, string> = {
    "해": "묘", "묘": "묘", "미": "묘",
    "인": "오", "오": "오", "술": "오",
    "사": "유", "유": "유", "축": "유",
...
export function getTwelveShinsalBySettings(params: {
  baseBranch: string;     // 기준 지지(연지 또는 일지)
  targetBranch: string;   // 대상 지지(연/월/일/시주, 대운/세운/월운/일운 등)
  era: EraType;           // "고전" | "현대"
  gaehwa: boolean;        // 개화론 on/off
}): string {

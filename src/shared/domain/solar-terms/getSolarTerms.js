...
// Date(UTC) → JD(UTC)
export function dateToJDUTC(d) {
    const t = d.getTime();
    return t / ONE_DAY_MS + 2440587.5; // Unix epoch(UTC) → JD(UTC)
}
// JD(UTC) → Date(UTC)
export function jdUTCToDate(jdUTC) {
...
// 결과 단위: 초
// ─────────────────────────────────────────────────────────────────────────────
export function deltaTSeconds(year, month = 7) {
...
function angDiff(target, value) { const d = (target - value + 540) % 360 - 180; return d; }
function unwrapToRef(a, ref) { while (a - ref > 180)
    a -= 360; while (a - ref < -180)
    a += 360; return a; }
function jdUTCtoJDTT(jdUTC) {
    const [y, m] = jdToCalendarGregorian(jdUTC);
    return jdUTC + deltaTSeconds(y, Math.round(m)) / DAYSEC;
}
// VSOP 평가 (단위: rad for L,B; AU for R)
function evalVSOP(series, T) {
    let sum = 0, Tp = 1;
...
export function getSunApparentLongitudeDeg(jdUTC) {
...
// ─────────────────────────────────────────────────────────────────────────────
export function findSolarTermUTC(year, targetDeg, lon) {

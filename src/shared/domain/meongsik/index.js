...
export function toCorrected(ms) {
    // birthDay: "19961229"
    ...
    if (isNaN(raw.getTime()))
        throw new Error(`Invalid date: ${ms.birthDay} ${ms.birthTime}`);
    return getCorrectedDate(raw, ms.birthPlace?.lon);
}
export function toDayStem(ms) {
    const d = toCorrected(ms);
    const gz = getDayGanZhi(d, ms.mingSikType ?? "자시");
    if (!gz)
        throw new Error(`getDayGanZhi returned undefined for ${d.toISOString()}`);
    const stem = normalizeStem(gz);
    if (!stem)
        throw new Error(`Invalid day stem from ${gz}`);
    return stem;
}

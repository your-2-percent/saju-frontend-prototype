...
import { UNASSIGNED_KEY, UNASSIGNED_LABELS } from "@/shared/domain/myeongsikList/model/constants";
import type { FolderFavMap, GroupedMap } from "@/shared/domain/myeongsikList/model/types";
...
export function moveByDnD(params: {
  ...
  const dstKey = destination.droppableId === UNASSIGNED_KEY ? UNASSIGNED_KEY : destination.droppableId;
  ...
  const insertIndex = Math.max(0, Math.min(destination.index, dstArr.length));
  if (srcKey === dstKey) {
...
export function unsetFolderForFolder(params: {
  ...
  const next = list.map((it) => {
    if ((it.folder ?? "").trim() !== target) return it;
    changedIds.push(String(it.id));
    return { ...it, folder: "" };
  });

  return { nextList: next, changedIds };
}

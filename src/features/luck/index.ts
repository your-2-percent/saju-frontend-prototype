...
const gzIndex = (gz: string) => SIXTY.indexOf(gz);
export const splitGanzhi = (gz: string) => {
  const i = gzIndex(gz);
  if (i < 0) throw new Error(`Unknown 간지: ${gz}`);
  return { stem: gz[0], branch: gz[1] } as const;
};

// 세운 10개 — 입춘 직후 1분 샘플
export function buildSeunListFromStart(firstBlockYear: number, count = 10) {
  const out: Array<{ year: number; yearGZ: string }> = [];
...
export function buildWolunListByTerms(yearForIpchun: number) {
  // 입춘~소한 순서(라벨: 2~12월, 마지막 1월은 다음 해)
  const PLAN: Array<{ label: string; y: number; deg: number }> = [
...
// 표시용 라벨: "YYYY–YYYY+9"
export const yearsLabel10 = (startAt: Date) =>
  `${startAt.getFullYear()}–${startAt.getFullYear() + 9}`;

...
/** 대운 리스트 생성 (기존 그대로) */
export function useDaewoonList(ms, hourTable = "자시", untilYears = 100) {
 ...
 * (이진탐색으로 견고하게)
 */
export function activeIndexAtOrFirst(arr, ref) {
    if (!arr.length)
        return -1;
    const t = ref.getTime();
    // arr 는 시간 오름차순이라고 가정.
    ...
    if (ans === -1)
        return 0; // 첫 칸 이전 → 0 강제
    if (ans >= arr.length)
        return arr.length - 1; // 방어
    return ans;
}
/** 훅: 리스트 + 활성 인덱스/아이템 한 번에 */
export function useDaewoonActive(ms, ref, hourTable = "자시", untilYears = 100) {
...
export function daewoonAge(birth, at) {

...
/* ===== 컴포넌트 ===== */
export default function DaewoonList({ data, activeIndex, onSelect, }) {
    const list = useDaewoonList(data); // [{ at: Date, gz: "갑자" }, ...]
    const dayStem = toDayStem(data);
    // ✅ 전역 설정 (SajuChart와 동일한 소스)
    const settings = useSettingsStore((s) => s.settings);
    const { charType, thinEum, showSipSin, showSibiUnseong, showSibiSinsal, sinsalBase, // "일지" | "연지"
    ...
    const baseBranchForShinsal = (sinsalBase === "일지"
        ? getDayGanZhi(birth, rule).charAt(1)
        : getYearGanZhi(birth, lon).charAt(1));
    const calcUnseong = (branch) => showSibiUnseong ? getTwelveUnseong(dayStem, branch) : null;
    const calcShinsal = (targetBranch) => showSibiSinsal

...
const HOUR = 60 * MIN;
const DAY = 24 * HOUR;
function addCalendarYears(base, years) {
...
export function roundToMinute(d) {
    const r = new Date(d);
    r.setSeconds(0, 0); // ✅ 초와 밀리초만 날리기
    return r;
}
/** 12 절(節)의 황경(°) 목록 (立春=315°, … 大雪=255°, 小寒=285°) */
const JIE_DEGREES = [315, 345, 15, 45, 75, 105, 135, 165, 195, 225, 255, 285];
/** 절기(UTC Date 그 자체를 그대로 사용) */
export function findPrevJie(birth) {
    const y = birth.getFullYear();
    const c = [];
    for (const yy of [y - 1, y])
        for (const deg of JIE_DEGREES) {
            const dt = findSolarTermUTC(yy, deg);
            if (dt.getTime() <= birth.getTime())
                c.push(dt);
        }
    if (!c.length)
        throw new Error("직전 절기 없음");
    c.sort((a, b) => b.getTime() - a.getTime());
    return roundToMinute(c[0]);
}
export function findNextJie(birth) {
    const y = birth.getFullYear();
    const c = [];
    ...
    if (!c.length)
        throw new Error("다음 절기 없음");
    c.sort((a, b) => a.getTime() - b.getTime());
    return roundToMinute(c[0]);
}
function firstSijuOffsetMs(birth, dir, table) {
    const h = birth.getHours();
    const startHour = table === "자시"
...
/** 시주 묘운 스케줄 (2h→10d) */
export function buildSijuSchedule(natal, natalHourGZ, dir, untilYears = 120, hourTable = "자시") {
...
/** 자/인/해/축 트리거 */
export function dayChangeTrigger(rule, dir) {
    const target = dir === "forward" ? (rule === "자시일수론" ? "자" : "인")
        : (rule === "자시일수론" ? "해" : "축");
    return (branch) => branch === target;
}
/** 시→일 전환 */
export function buildIljuFromSiju(siju, natalDayGZ, dir, rule) {
...
export function getSolarTermBoundaries(natal) {
    ...
    const start = findSolarTermUTC(y, 315), end = findSolarTermUTC(y + 1, 315);
    return arr
        .filter(t => t.date >= start && t.date < end)
        .sort((a, b) => a.date.getTime() - b.date.getTime());
}
export function daewoonAge(birth, at, offset = 0) {
...
// 매핑
const KO_TO_HJ_STEM = { 갑: "甲", 을: "乙", 병: "丙", 정: "丁", 무: "戊", 기: "己", 경: "庚", 신: "辛", 임: "壬", 계: "癸" };
const KO_TO_HJ_BRANCH = { 자: "子", 축: "丑", 인: "寅", 묘: "卯", 진: "辰", 사: "巳", 오: "午", 미: "未", 신: "申", 유: "酉", 술: "戌", 해: "亥" };
const toHJStem = (ch) => ((KO_TO_HJ_STEM)[ch] ?? ("甲乙丙丁戊己庚辛壬癸".includes(ch) ? ch : null));
const toHJBranch = (ch) => ((KO_TO_HJ_BRANCH)[ch] ?? ("子丑寅卯辰巳午未申酉戌亥".includes(ch) ? ch : null));
export function normalizeGZtoHJ(gz) {
    const s = toHJStem(gz[0]);
    const b = toHJBranch(gz[1]);
    if (!s || !b)
        throw new Error(`Invalid GZ: ${gz}`);
    return `${s}${b}`;
}
export const eqGZ = (a, b) => normalizeGZtoHJ(a) === normalizeGZtoHJ(b);
export function buildWolju(natal, // 보정시 (초/밀리초 0, 절기용)
natalMonthGZ, // 출생 시점 월주 간지
dir, untilYears = 120, lon) {
    // ───── 보조: 시간→지지, 지지/천간 스텝 ─────
    ...
    // 간지 한 칸 이동(원본 유지)
    function stepGZ(base, direction, step) {
    ...
    let relevantSolarTerm;
    if (dir === 'forward') {
        const i = allTerms.findIndex(t => t.date >= natal);
        if (i >= 0)
            relevantSolarTerm = allTerms[i].date; // 다음 절입
    }
    else {
        const past = allTerms.filter(t => t.date <= natal);
        if (past.length)
            relevantSolarTerm = past[past.length - 1].date; // 직전 절입
    }
    // ───── 복원: 네 ‘역행 전용’ 계산 그대로 + 리버스한 순행 ─────
    function getFirstSijuChangeRestored(dt, solarTermTime) {
    ...
    try {
        ...
        const isAdjacent = (dir === "forward" && refMonthAtBirthHJ !== refMonthAtBirth) ||
...
/** 월→년 전환 (월지 트리거) */
export function buildYeonjuFromWolju(wolju, natalYearGZ, dir, rule, natal) {
    const isTrigger = dayChangeTrigger(rule, dir);
    let cur = natalYearGZ;
    const events = [];
...
/** 보정 전 로컬시(분 유지) */
export function rawBirthLocal(ms) {
...
// myoun.ts
export function parseBirthLocal(ms) {
...
/** 원국 4기둥 */
export function computeNatalPillars(ms, hourTable) {
...
/** 120년 콘솔 */
export function logWolju120(wolju) {

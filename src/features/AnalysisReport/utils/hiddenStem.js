...
function expandStemKeys(src) {
    const out = { ...src };
    for (const k of Object.keys(src)) {
        const alt = /[甲乙丙丁戊己庚辛壬癸]/.test(k) ? toKoStem(k) : toCnStem(k);
        if (alt && out[alt] === undefined)
            out[alt] = src[k];
    }
    return out;
}
function expandBranchKeys(src) {
    const out = { ...src };
    for (const k of Object.keys(src)) {
...
/** 지지 메인 오행 (원본: 한글) */
const BRANCH_MAIN_ELEMENT_KO = {
    "자": "수", "축": "토", "인": "목", "묘": "목", "진": "토", "사": "화",
    "오": "화", "미": "토", "신": "금", "유": "금", "술": "토", "해": "수",
};
/** 하건충 (원본: 한글) */
const BRANCH_HIDDEN_STEMS_HGC_KO = {
...
/** 고전 (원본: 한글) */
const BRANCH_HIDDEN_STEMS_CLASSIC_KO = {
...
export const STEM_TO_ELEMENT = expandStemKeys(STEM_TO_ELEMENT_KO);
export const BRANCH_MAIN_ELEMENT = expandBranchKeys(BRANCH_MAIN_ELEMENT_KO);
export const BRANCH_HIDDEN_STEMS_HGC = expandBranchKeys(BRANCH_HIDDEN_STEMS_HGC_KO);
export const BRANCH_HIDDEN_STEMS_CLASSIC = expandBranchKeys(BRANCH_HIDDEN_STEMS_CLASSIC_KO);

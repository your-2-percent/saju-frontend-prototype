...
function getBand(pct) {
    const x = clamp01(pct);
    return (BANDS.find(b => x >= b.min && x < b.max)?.name ?? "중화");
}
function dayElement(pillars) {
    const dayStem = pillars?.[2]?.charAt(0) ?? "";
    return STEM_TO_ELEMENT[dayStem] ?? null;
}
function monthBranch(pillars) {
    const mb = pillars?.[1]?.charAt(1) ?? "";
    return mb || null;
}
function findOfficer(target) {
    // x such that KE[x] === target
    const entries = Object.entries(KE);
...
function lightElementScore(pillars) {
    ...
    for (const gz of pillars) {
        ...
        const s = gz.charAt(0);
        const b = gz.charAt(1);
        const se = STEM_TO_ELEMENT[s];
        const be = BRANCH_MAIN_ELEMENT[b];
        if (se)
            acc[se] += 10;
        if (be)
            acc[be] += 6;
...
function overallFromTenGod(totals) {
    if (!totals || totals.length === 0)
        return null;
    const sum = totals.reduce((a, b) => a + (b?.value ?? 0), 0) || 0;
    if (sum <= 0)
        return null;
    const get = (name) => totals.find(t => t.name === name)?.value ?? 0;
    const bigyup = get("비겁");
 ...
 * 타입별 스코어러
 * ──────────────────────────────────────────────────────────────────────────── */
function scoreEokbu(dEl, band, _elemScore, reasons) {
...
function scoreChoHu(mb, reasons) {
    if (!mb)
...
function scoreTongGwan(elemScore, reasons) {
    const s = { 목: 0, 화: 0, 토: 0, 금: 0, 수: 0 };
...
function scoreByeongYak(elemScore, reasons) {
    const s = { 목: 0, 화: 0, 토: 0, 금: 0, 수: 0 };
    const entries = Object.entries(elemScore);
 ...
 * 메인: 용신 계산
 * ──────────────────────────────────────────────────────────────────────────── */
export function computeYongshin(pillars, tenGodTotals) {
    ...
    const reasons = new Map(["목", "화", "토", "금", "수"].map(k => [k, []]));
    // 1) 억부
    if (dEl) {
        const s1 = scoreEokbu(dEl, band, elemScore, reasons);
        Object.keys(score).forEach(k => { score[k] += s1[k] ?? 0; });
    }
    // 2) 조후
    const s2 = scoreChoHu(mb, reasons);
    Object.keys(score).forEach(k => { score[k] += s2[k] ?? 0; });
    // 3) 통관
    const s3 = scoreTongGwan(elemScore, reasons);
    Object.keys(score).forEach(k => { score[k] += s3[k] ?? 0; });
    // 4) 병약
    const s4 = scoreByeongYak(elemScore, reasons);
    Object.keys(score).forEach(k => { score[k] += s4[k] ?? 0; });
    // === 4.5) 밸런스 우선 레이어 (Neutral zone 중심) =========================
    const prim = primaryOfDay(dEl);
    // 십성 합계 맵 (없으면 0)
    const tgMap = { 비겁: 0, 식상: 0, 재성: 0, 관성: 0, 인성: 0 };
    ...
    if (isNeutral && prim) {
        ...
        const officerHeavy = tgMap["관성"] >= 30 ||
            tgMap["관성"] >= Math.max(tgMap["비겁"], tgMap["식상"], tgMap["재성"], tgMap["인성"]) + 5;
        if (overallPct <= 50) {
            // 약측 중화 → 인성 > 비겁 > 식상 > 재성 > 관성
            score[prim.resource] += 14;
            reasons.get(prim.resource)?.push("밸런스: 중화-약측 → 인성 우선");
            score[prim.peer] += 10;
            reasons.get(prim.peer)?.push("밸런스: 중화-약측 → 비겁 보강");
            score[prim.leak] += 6;
            reasons.get(prim.leak)?.push("밸런스: 중화-약측 → 식상 보조");
            score[prim.wealth] += 3;
            reasons.get(prim.wealth)?.push("밸런스: 중화-약측 → 재성 소량");
            // 관성 과다 억제
            ...
            score[prim.leak] += 14;
            reasons.get(prim.leak)?.push("밸런스: 중화-강측 → 식상 우선");
            score[prim.wealth] += 10;
            reasons.get(prim.wealth)?.push("밸런스: 중화-강측 → 재성 보조");
            if (prim.officer) {
                score[prim.officer] += 6;
                reasons.get(prim.officer)?.push("밸런스: 중화-강측 → 관성 완충");
            }
            score[prim.resource] += 3;
            reasons.get(prim.resource)?.push("밸런스: 중화-강측 → 인성 소량");
        ...
        if (dEl === "토" && overallPct <= 50 && officerHeavy) {
            // 화(인성) 추가, 목(관성) 추가 감점
            score["화"] += 4;
            reasons.get("화")?.push("밸런스: 토일간·관성강 → 화(인성) 추가 가점");
    ...
    // 최종 후보 배열
    const ordered = Object.keys(score)
        .map(el => ({
        element: el,
        score: Math.round(score[el] * 10) / 10,
        via: collectVia(el, scoreChoHu(mb, new Map()), scoreTongGwan(elemScore, new Map()), scoreByeongYak(elemScore, new Map())),
        reasons: (reasons.get(el) ?? []).slice(),
        tenGodHints: tenHints[el],
    }))
        .sort((a, b) => b.score - a.score);
    // 대표 타입 결정
    const chosenType = chooseType(band, scoreChoHu(mb, new Map()), scoreTongGwan(elemScore, new Map()), scoreByeongYak(elemScore, new Map()));
    // debug.tenGodTotals를 타입 안전하게 구성
    const initTenGods = {
        비겁: 0, 식상: 0, 재성: 0, 관성: 0, 인성: 0,
    };
    const tgTotals = tenGodTotals
...
function collectVia(el, sChoHu, sTong, sBy) {
    const via = [];
    if ((sChoHu[el] ?? 0) > 0)
        via.push("조후");
    if ((sTong[el] ?? 0) > 0)
...
function chooseType(band, sChoHu, sTong, sBy) {

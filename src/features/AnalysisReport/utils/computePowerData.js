...
const stemPolarity = (s) => (YIN_STEMS.has(s) ? "음" : "양");
const branchPolarity = (b) => {
    const base = BRANCH_YANG.has(b) ? "양" : "음";
 ...
 * ========================= */
function projectionPercent(dist, samePol) {
    if (dist === 0)
        return samePol ? 1.0 : 0.8;
    if (dist === 1)
        return samePol ? 0.6 : 0.5;
    return samePol ? 0.3 : 0.2;
}
const LV_ADJ = { 왕: +0.15, 상: +0.10, 휴: -0.15, 수: -0.20, 사: -0.25 };
const SHENG_NEXT = { 목: "화", 화: "토", 토: "금", 금: "수", 수: "목" };
const KE = { 목: "토", 화: "금", 토: "수", 금: "목", 수: "화" };
function relationLevel(me, nb) {
    if (me === nb)
        return "왕";
    if (SHENG_NEXT[nb] === me)
        return "상"; // 이웃이 나를 生
 ...
 * 공개 API
 * ========================= */
export function computePowerData(pillars, hiddenMode = "hgc", hiddenStemSetting = "all", debug = false, useHarmonyOverlay = false, 
/** ✨ 추가: 현대/고전 가중치 모드 */
criteriaMode = "modern") {
    const out = computePowerDataDetailed(pillars, hiddenMode, hiddenStemSetting, debug, useHarmonyOverlay, criteriaMode);
    return out.totals;
}
// ─────────────────────────────────────────────────────────────
// 구현부 (두 시그니처 공용)
export function computePowerDataDetailed(...args) {
    ...
    const opt = (() => {
        if (isOptions) {
            ...
            const [pillars, hiddenMode = "hgc", hiddenStemSetting = "all", debug = false, useHarmonyOverlay = false, criteriaMode = "modern"] = args;
            return {
                pillars,
                dayStem: undefined,
                mode: hiddenMode,
    ...
    const { pillars, dayStem: dayStemOverride, mode: hiddenMode, hidden: hiddenStemSetting, debug, useHarmonyOverlay, criteriaMode, luck } = opt;
    ...
    if (pillarsKo.length !== 4 || pillarsKo.some(gz => gz.length < 2)) {
        const zeros = ["비겁", "식상", "재성", "관성", "인성"].map(n => ({ name: n, value: 0, color: "#999" }));
        return {
            totals: zeros,
            perTenGod: {
    ...
    const dayEl = dayStem === "갑" || dayStem === "을" ? "목" :
    ...
    const stemParts = [{ val: 0 }, { val: 0 }, { val: 0 }, { val: 0 }];
    const branchParts = [[], [], [], []];
    /* 2) 자리 점수(천간/지지) */
    for (let i = 0; i < 4; i++) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (nbElV)
            adj += LV_ADJ[relationLevel(el, nbElV)];
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (nbElU)
            adj += LV_ADJ[relationLevel(el, nbElU)];
        const sideIdx = i === 0 ? [1] : i === 3 ? [2] : [i - 1, i + 1];
        for (const j of sideIdx) {
        ...
        if (baseHere !== 0 && adj !== 0) {
            ...
            elementScore[el] += delta;
            if (branchParts[i].length > 0)
                branchParts[i][0].val += delta;
        }
    }
    /* (옵션) 합충형파해 오버레이 */
    ...
    function addLuckGZ(raw, kind) {
        ...
        const elS = STEM_TO_ELEMENT[s];
        const elB = BRANCH_MAIN_ELEMENT[b];
        const sumBase = Math.max(1, (elementScore.목 ?? 0) + (elementScore.화 ?? 0) + (elementScore.토 ?? 0) + (elementScore.금 ?? 0) + (elementScore.수 ?? 0));
    ...
    if (luck && luck.tab !== "원국") {
        ...
        if (luck.tab === "세운" || luck.tab === "월운")
            addLuckGZ(luck.se, "se");
        if (luck.tab === "월운")
            addLuckGZ(luck.wol, "wol");
    }
    /* 4) 오행→십신 대분류 */
    const tenAcc = { 비겁: 0, 식상: 0, 재성: 0, 관성: 0, 인성: 0 };
    Object.entries(elementScore).forEach(([el, v]) => {
    ...
    function pickSubtype(el, srcPol) {
        const SHENG_PREV_LOCAL = { 화: "목", 토: "화", 금: "토", 수: "금", 목: "수" };
        const major = el === dayEl ? "비겁" :
    ...
    const addSub = (el, pol, val) => {
        const { sub } = pickSubtype(el, pol);
        subAcc[sub] = (subAcc[sub] ?? 0) + val;
    };
    // 원국 파트 반영
    stemParts.forEach(p => { if (p.el && p.pol && p.val)
        addSub(p.el, p.pol, p.val); });
    ...
    const totals = order.map((name, i) => ({ name, value: pct[i], color: colors[name] }));
    /* 7) 소분류를 대분류 값으로 리스케일 */
    const perTenGod = {

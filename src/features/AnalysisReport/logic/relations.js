...
const POS_LABELS = ["연", "월", "일", "시"];
const POS = { year: 0, month: 1, day: 2, hour: 3 };
...
// 형
const TRIAD_SHAPE_GROUPS = [
    { name: "인사신", members: ["인", "사", "신"] },
    { name: "축술미", members: ["축", "술", "미"] },
];
const BR_SANGHYEONG_LABELS = [
    { pair: ["인", "신"], label: "인신상형" },
    { pair: ["인", "사"], label: "인사상형" },
    { pair: ["사", "신"], label: "사신상형" },
    { pair: ["술", "미"], label: "술미상형" },
...
/* ───────── 유틸 ───────── */
function labelForPair(table, a, b) {
...
function pushUnique(arr, tag) {
    if (!arr.includes(tag))
        arr.push(tag);
}
const posMask = (idxs) => [...idxs].sort((a, b) => a - b).map(i => POS_LABELS[i]).join("X");
const isYearHourPair = (i, j) => (i === 0 && j === 3) || (i === 3 && j === 0);
function selectAllPairs(posA, posB) {
    const raw = [];
    for (const i of posA)
        for (const j of posB) {
            if (i === j)
...
function selectAllPairsSame(pos) {
    const raw = [];
...
function finalizeBuckets(out) {
    for (const k of BUCKET_KEYS) {
        const raw = (out[k] ?? []);
        const cleaned = raw
            .map(s => s && s.normalize("NFKC"))
            .filter((t) => !!t && !isNoneTag(t)); // ← '#없음' 전부 제거
        // 데이터 단계에서는 절대 '#없음'을 넣지 않는다(빈 배열 유지)
        out[k] = cleaned;
    }
    return out;
}
export function buildHarmonyTags(pillarsRaw, opts = {}) {
    ...
    // 천간 합/충 (인접만)
    const addStemPairAdjacent = (a, b, label, bucket) => {
    ...
    // ── 삼합/반합 ──
    const findFirstIdxPerType = (types) => {
        const used = new Set();
        const idxs = [];
        for (const t of types)
            for (let i = 0; i < brs.length; i++) {
                if (brs[i] === t && !used.has(i)) {
                    used.add(i);
                    idxs.push(i);
                    break;
                }
    ...
    SANHE_GROUPS.forEach(({ name, members, wang }) => {
        ...
        if (uniq === 3) {
            ...
            if (idxs.length === 3)
                pushUnique(out.jijiSamhap, `#${posMask(idxs)}_${name}_삼합`);
        }
        else if (uniq === 2) {
    ...
    BANGHAP_GROUPS.forEach(({ name, members }) => {
        ...
        const idxs = findFirstIdxPerType(members);
        if (idxs.length === 3)
            pushUnique(out.jijiBanghap, `#${posMask(idxs)}_${name}_방합`);
    });
    // ── 지지: 육합/충/파/해/원진/귀문 ──
    const addBranchPairsAllByLabel = (a, b, label, bucket) => {
    ...
    [["인", "사", "신"], ["축", "술", "미"]].forEach(group => {
        ...
        if (uniqHits.length === 3)
            pushUnique(out.jijiHyeong, `#${posMask(uniqHits)}_${group.join("")}_삼형`);
    });
    ...
    if (emitGanjiAmhap) {
        for (let i = 0; i < 4; i++) {
            ...
            if (GANJI_AMHAP_SET.has(pair))
                pushUnique(out.ganjiAmhap, `#${posToJuLabel(POS_LABELS[i])}_${pair}_암합`);
        }
    }
    return finalizeBuckets(out);
}
export function buildAllRelationTags(input, opts = {}) {
    ...
    for (const [kind, rawLuck] of lucks) {
        ...
        // 운 자체 간지암합(같은 기둥)
        {
            const pair = `${ls}${lb}`;
            if (GANJI_AMHAP_SET.has(pair))
                pushUnique(out.ganjiAmhap, `#${kind}_${pair}_암합`);
        }
        // 포지션별 1:1 상호작용
        for (let i = 0; i < 4; i++) {
            const gz = natalKo[i] ?? "";
            ...
            if (hap)
                pushUnique(out.cheonganHap, `#${kind}X${pos}_${hap}`);
            const cgChung = labelForPair(STEM_CHUNG_LABELS, ls, ns);
            if (cgChung)
                pushUnique(out.cheonganChung, `#${kind}X${pos}_${cgChung}`);
            const yukhap = labelForPair(BR_YUKHAP_LABELS, lb, nb);
            if (yukhap)
                pushUnique(out.jijiYukhap, `#${kind}X${pos}_${yukhap}`);
            const brChung = labelForPair(BR_CHUNG_LABELS, lb, nb);
            if (brChung)
                pushUnique(out.jijiChung, `#${kind}X${pos}_${brChung}`);
            const pa = labelForPair(BR_PA_LABELS, lb, nb);
            if (pa)
                pushUnique(out.jijiPa, `#${kind}X${pos}_${pa}`);
            const hae = labelForPair(BR_HAE_LABELS, lb, nb);
            if (hae)
                pushUnique(out.jijiHae, `#${kind}X${pos}_${hae}`);
            const wonjin = labelForPair(BR_WONJIN_LABELS, lb, nb);
            if (wonjin)
                pushUnique(out.jijiWonjin, `#${kind}X${pos}_${wonjin}`);
            const gwimun = labelForPair(BR_GWIMUN_LABELS, lb, nb);
            if (gwimun)
                pushUnique(out.jijiGwimun, `#${kind}X${pos}_${gwimun}`);
            // 형
            const sang = labelForPair(BR_SANGHYEONG_LABELS, lb, nb);
            if (sang)
                pushUnique(out.jijiHyeong, `#${kind}X${pos}_${sang}`);
            else {
                const zamyo = labelForPair(BR_ZAMYO_HYEONG_LABELS, lb, nb);
                if (zamyo)
                    pushUnique(out.jijiHyeong, `#${kind}X${pos}_${zamyo}`);
        ...
        for (const g of SANHE_GROUPS) {
            ...
            if (new Set(hits.map(h => h.b)).size === 2) {
                const [p1, p2] = hits.map(h => h.pos).sort((a, b) => (["시", "일", "월", "연"].indexOf(a) - ["시", "일", "월", "연"].indexOf(b)));
 ...
 * ============================================================ */
export function applyHarmonyOverlay(pillarsRaw, elementScore) {
    const pillarsKo = pillarsRaw.map(normalizeGZ);
    const brs = pillarsKo.map(gz => gzBranch(gz));
    const monthIdx = POS.month;
    const monthB = brs[monthIdx];
    const findIndices = (targets) => {
        const idxs = [];
        for (let i = 0; i < 4; i++) {
            const b = brs[i];
            if (b && targets.has(b))
    ...
    for (const g of SANHE_GROUPS) {
        ...
        if (idxs.length === 3) {
            ...
            if (monthIn) {
                if (isClustered3(idxs))
                    elementScore[g.out] += wangAtMonth ? 10 : 8;
                else
                    elementScore[g.out] += 5;
            }
            else {
        ...
        if (idxs.length === 2 && idxs.includes(monthIdx)) {
            ...
            const wangAtMonth = monthB === g.wang;
            if (other === POS.day && wangAtMonth)
                elementScore[g.out] += 8;
            else if (other === POS.year && wangAtMonth)
                elementScore[g.out] += 6;
            else if (other === POS.hour)
                elementScore[g.out] += wangAtMonth ? 4 : 2;
        }
    }
    // 방합(3지)
    for (const g of BANGHAP_GROUPS) {
        ...
        if (idxs.length === 3) {
            ...
            const wangAtMonth = monthB === g.wang;
            if (!monthIn)
                elementScore[g.out] += 2;
            else if (isClustered3(idxs))
                elementScore[g.out] += wangAtMonth ? 8 : 6;
            else
                elementScore[g.out] += 4;
        }
    }
}
/* ============================================================
 ...
 * ============================================================ */
export function mergeRelationTags(...entries) {
    const base = {
        cheonganHap: [], cheonganChung: [],
        jijiSamhap: [], jijiBanhap: [], jijiBanghap: [],
        jijiYukhap: [], jijiChung: [], jijiHyeong: [],
...
/* ───────── (참고) 오행 생/극 ───────── */
export const SHENG_NEXT = { 목: "화", 화: "토", 토: "금", 금: "수", 수: "목" };
export const SHENG_PREV = { 화: "목", 토: "화", 금: "토", 수: "금", 목: "수" };
export const KE = { 목: "토", 화: "금", 토: "수", 금: "목", 수: "화" };
export const KE_INV = { 목: "금", 화: "수", 토: "목", 금: "화", 수: "토" };
export const LV_ADJ = { 왕: +0.15, 상: +0.10, 휴: -0.15, 수: -0.20, 사: -0.25 };

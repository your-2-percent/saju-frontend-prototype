const POS_WEIGHT = { 2: 4, 1: 3, 3: 2, 0: 1 };
...
function findBestPosForBranch(branch, natal) {
    const positions = [idx.day, idx.month, idx.hour, idx.year]; // 가중치 순
    for (const p of positions) {
        if (last(natal[p]) === branch)
            return { pos: p, weight: POS_WEIGHT[p] ?? 0 };
    }
    return null;
}
function findBestPosForStem(stem, natal) {
    const positions = [idx.day, idx.month, idx.hour, idx.year];
    for (const p of positions) {
        if (first(natal[p]) === stem)
            return { pos: p, weight: POS_WEIGHT[p] ?? 0 };
    }
    return null;
}
const DUP_POS_RE = /^#(연지|월지|일지|시지)X\1_/;
const isSamePosProductLabel = (s) => DUP_POS_RE.test(s);
/** (name,pos) 단위로 중복 제거하면서, 동일 (name,pos)일 때 weight 최대값 유지 */
function uniqKeepMaxPerPos(items) {
    const map = new Map(); // key = `${name}@${pos}`
    for (const it of items) {
        if (isSamePosProductLabel(it.name))
            continue; // ← 같은 자리끼리 곱셈은 버림
        const key = `${it.name}@${it.pos}`;
        const ex = map.get(key);
        if (!ex || it.weight > ex.weight)
            map.set(key, it);
    }
    return Array.from(map.values()).sort((a, b) => b.weight !== a.weight ? b.weight - a.weight : a.name.localeCompare(b.name, "ko"));
}
 ...
 * 표기 헬퍼: #좌측X우측_신살
 * ──────────────────────────────────────────────────────────────────────────── */
const posKo = (p) => (p === 0 ? "연지" : p === 1 ? "월지" : p === 2 ? "일지" : "시지");
const srcKo = (s) => (s === "dae" ? "대운" : s === "se" ? "세운" : "월운");
// (원국) 일간 × (지지 위치)
const labelSB_at = (tag, atPos) => `#일간X${posKo(atPos)}_${tag}`;
// (원국) 월지 × (다른 위치: 천간/지지 상관없이 그 위치)
const labelMonth_withPos = (tag, otherPos) => otherPos === idx.month ? `#월지_${tag}` : `#월지X${posKo(otherPos)}_${tag}`;
// (원국) 위치 × 위치 (쌍)
const labelPair_at = (tag, p1, p2) => `#${posKo(p1)}X${posKo(p2)}_${tag}`;
// (원국) 공망: (일간|연간) × 트리거 위치
const labelVoid_at = (basis, atPos) => `#${basis === "day" ? "일간" : "연간"}X${posKo(atPos)}_공망`;
// (원국) 일주 단독
const labelIlju = (tag) => `#일주_${tag}`;
// (운) 일간 × (대운|세운|월운)
const labelLuck_SB = (tag, src) => `#일간X${srcKo(src)}_${tag}`;
// (운) 월지 × (대운|세운|월운)
const labelLuck_Month = (tag, src) => `#월지X${srcKo(src)}_${tag}`;
// (운) (대운|세운|월운) × 원국 위치
const labelLuck_SrcWithPos = (tag, src, natPos) => `#${srcKo(src)}X${posKo(natPos)}_${tag}`;
// (운) 공망: (일간|연간) × (대운|세운|월운)
const labelLuck_Void = (src, basis) => `#${basis === "day" ? "일간" : "연간"}X${srcKo(src)}_공망`;
// (운) 삼재: (일지|연지) × (세운)
const labelLuck_Samjae = (src, basis) => `#${basis === "day" ? "일지" : "연지"}X${srcKo(src)}_삼재`;
// (운) 상문/조객: 연지 × 세운
const labelLuck_SangJoe = (src, kind) => `#연지X${srcKo(src)}_${kind}`;
/* ────────────────────────────────────────────────────────────────────────────
 * 파서/라벨 기타
 * ──────────────────────────────────────────────────────────────────────────── */
...
function parseGZ(raw) {
    ...
    for (const ch of chars) {
        if (!stem && (STEMS_KO.has(ch) || STEMS_HJ.has(ch)))
            stem = ch;
        if (BRANCHES_KO.has(ch) || BRANCHES_HJ.has(ch))
            branch = ch; // 마지막 지지 유지
    }
    return stem && branch ? { stem, branch } : null;
}
function natalBranches(natal) {
...
/** 무순서 쌍 포함 여부 */
function isInPairList(list, a, b) {
...
function getVoidPair(pillar) {
    for (const row of 공망표)
        if (row.set.has(pillar))
            return row.voids;
    return null;
}
function getSamjaeYears(branch) {
    for (const g of 삼재그룹)
        if (g.group.has(branch))
            return Array.from(g.years);
    return null;
}
/* ────────────────────────────────────────────────────────────────────────────
 * 메인
 * ──────────────────────────────────────────────────────────────────────────── */
export function buildShinsalTags({ natal, daewoon, sewoon, wolwoon, basis, }) {
    ...
    if (금여록[dStem]) {
        ...
        if (best)
            natalGoodPos.push({ name: labelSB_at("금여록", best.pos), weight: best.weight, pos: best.pos });
    }
    // 천을귀인
    if (천을귀인[dStem]) {
        for (const b of 천을귀인[dStem]) {
            const best = findBestPosForBranch(b, natal);
            if (best)
                natalGoodPos.push({ name: labelSB_at("천을귀인", best.pos), weight: best.weight, pos: best.pos });
        }
    }
    // 학당귀인
    if (학당귀인[dStem]) {
        ...
        const best = findBestPosForBranch(b, natal);
        if (best)
            natalGoodPos.push({ name: labelSB_at("학당귀인", best.pos), weight: best.weight, pos: best.pos });
    }
    // 문창귀인
    if (문창귀인[dStem]) {
        const b = 문창귀인[dStem];
        const best = findBestPosForBranch(b, natal);
        if (best)
            natalGoodPos.push({ name: labelSB_at("문창귀인", best.pos), weight: best.weight, pos: best.pos });
    }
    // 태극귀인
    for (const g of 태극귀인) {
        if (g.stems.includes(dStem)) {
            for (const b of g.branches) {
                const best = findBestPosForBranch(b, natal);
                if (best)
                    natalGoodPos.push({ name: labelSB_at("태극귀인", best.pos), weight: best.weight, pos: best.pos });
            }
        }
    }
    // 관귀학관
    for (const g of 관귀학관) {
        if (g.stems.includes(dStem)) {
            ...
            const best = findBestPosForBranch(b, natal);
            if (best)
                natalGoodPos.push({ name: labelSB_at("관귀학관", best.pos), weight: best.weight, pos: best.pos });
        }
    }
    // 천주성
    for (const g of 천주성) {
        if (g.stems.includes(dStem)) {
            ...
            const best = findBestPosForBranch(b, natal);
            if (best)
                natalGoodPos.push({ name: labelSB_at("천주성", best.pos), weight: best.weight, pos: best.pos });
        }
    }
    // 암록
    if (암록[dStem]) {
        ...
        const best = findBestPosForBranch(b, natal);
        if (best)
            natalGoodPos.push({ name: labelSB_at("암록", best.pos), weight: best.weight, pos: best.pos });
    }
    ...
    for (const rule of 천덕귀인_byMonth) {
        if (rule.month !== mBranch)
            continue;
        if ("needBranch" in rule) {
            const b = rule.needBranch;
            const best = findBestPosForBranch(b, natal);
            if (best)
                natalGoodPos.push({ name: labelMonth_withPos("천덕귀인", best.pos), weight: best.weight, pos: best.pos });
        }
    ...
    // 삼기귀인(표기: #<위치>X<위치>X<위치>_삼기귀인(천상|인중|지하))
    {
        ...
        for (const t of triples) {
            for (const c of combos) {
                ...
                const weight = (c.includes(idx.day) ? POS_WEIGHT[idx.day] : 0) ||
    ...
    if (양인[dStem]) {
        ...
        const best = findBestPosForBranch(b, natal);
        if (best)
            natalBadPos.push({ name: labelSB_at("양인", best.pos), weight: best.weight, pos: best.pos });
    }
    // 백호
    if (백호[dStem]) {
        const b = 백호[dStem];
        const best = findBestPosForBranch(b, natal);
        if (best)
            natalBadPos.push({ name: labelSB_at("백호", best.pos), weight: best.weight, pos: best.pos });
    }
    // 홍염살(흉)
    if (홍염살[dStem]) {
        const b = 홍염살[dStem];
        const best = findBestPosForBranch(b, natal);
        if (best)
            natalBadPos.push({ name: labelSB_at("홍염살", best.pos), weight: best.weight, pos: best.pos });
    }
    ...
    if (현침.stems.has(dStem)) {
        const positions = [idx.year, idx.month, idx.day, idx.hour];
        for (const p of positions) {
    ...
    const goodByPos = { si: [], il: [], yeon: [], wol: [] };
    const badByPos = { si: [], il: [], yeon: [], wol: [] };
    for (const it of goodPosDedup)
        goodByPos[posToKey(it.pos)].push(it.name);
    for (const it of badPosDedup)
        badByPos[posToKey(it.pos)].push(it.name);
    // ── 운(대운/세운/월운) ──
    const luckGoodDae = [];
    const luckGoodSe = [];
    const luckGoodWol = [];
    ...
    const luckBadSe = [];
    const luckBadWol = [];
    function pushLuck(arr, v) {
        if (!arr.includes(v))
            arr.push(v);
    }
    function matchLuckOne(src, luck) {
        ...
        // 길신 (일간×운)
        if (금여록[dStem] === Lb)
            pushLuck(good, labelLuck_SB("금여록", src));
        if (천을귀인[dStem]?.includes(Lb))
            pushLuck(good, labelLuck_SB("천을귀인", src));
        if (학당귀인[dStem] === Lb)
            pushLuck(good, labelLuck_SB("학당귀인", src));
        if (문창귀인[dStem] === Lb)
            pushLuck(good, labelLuck_SB("문창귀인", src));
        for (const g of 태극귀인)
            if (g.stems.includes(dStem) && g.branches.includes(Lb))
                pushLuck(good, labelLuck_SB("태극귀인", src));
        for (const g of 관귀학관)
            if (g.stems.includes(dStem) && g.branch === Lb)
                pushLuck(good, labelLuck_SB("관귀학관", src));
        for (const g of 천주성)
            if (g.stems.includes(dStem) && g.branch === Lb)
                pushLuck(good, labelLuck_SB("천주성", src));
        if (암록[dStem] === Lb)
            pushLuck(good, labelLuck_SB("암록", src));
        // 흉살 (일간×운)
        if (양인[dStem] === Lb)
            pushLuck(bad, labelLuck_SB("양인", src));
        if (백호[dStem] === Lb)
            pushLuck(bad, labelLuck_SB("백호", src));
        if (홍염살[dStem] === Lb)
            pushLuck(bad, labelLuck_SB("홍염살", src));
        ...
        for (const nb of branches) {
            if (isInPairList(천라지망_pairs, Lb, nb)) {
                ...
                if (pos !== undefined)
                    pushLuck(bad, labelLuck_SrcWithPos("천라지망", src, pos));
                break;
            }
        }
        // 현침(일간×운지)
        ...
        for (const rule of 천덕귀인_byMonth) {
            if (rule.month !== mBranch)
                continue;
            if ("needBranch" in rule && Lb === rule.needBranch)
                pushLuck(good, labelLuck_Month("천덕귀인", src));
            if ("needStem" in rule && Ls === rule.needStem)
                pushLuck(good, labelLuck_Month("천덕귀인", src));
        }
        // 월덕귀인(월지×운간)
        {
            const needStem = 월덕귀인_byMonthStem[mBranch];
            if (needStem && Ls === needStem)
                pushLuck(good, labelLuck_Month("월덕귀인", src));
        }
        // 천의성(월지×운지)
        const 천의매핑 = {
        ...
        if (천의매핑[mBranch] === Lb)
            pushLuck(good, labelLuck_Month("천의성", src));
        // 원진(운지+원국 지지) — #세운X<위치>_원진
        for (const nb of branches) {
            if (isInPairList(원진_pairs, Lb, nb)) {
                const pos = findBestPosForBranch(nb, natal)?.pos;
                if (pos !== undefined)
                    pushLuck(bad, labelLuck_SrcWithPos("원진", src, pos));
                break;
            }
        }
        // 귀문(운지+월/일지) — 연지 제외
        {
            // 일지 매칭
            if (isInPairList(귀문_pairs, Lb, dBranch))
                pushLuck(bad, labelLuck_SrcWithPos("귀문", src, idx.day));
            // 월지 매칭
            if (isInPairList(귀문_pairs, Lb, mBranch))
                pushLuck(bad, labelLuck_SrcWithPos("귀문", src, idx.month));
        }
        // 삼재(세운): (일지|연지) × 세운
        if (src === "se") {
            const baseBranch = samjaeBasis === "day" ? dBranch : yBranch;
            const yrs = baseBranch ? getSamjaeYears(baseBranch) : null;
            if (yrs && yrs.includes(Lb))
                pushLuck(bad, labelLuck_Samjae(src, samjaeBasis));
        }
        // 상문/조객(세운): 연지 × 세운
        if (src === "se") {
            const 표 = {
            ...
            if (p) {
                if (Lb === p.sang)
                    pushLuck(bad, labelLuck_SangJoe(src, "상문살"));
                if (Lb === p.joe)
                    pushLuck(bad, labelLuck_SangJoe(src, "조객살"));
            }
        }
    }

...
const MAP_D_HYOSIN  = D({ 갑:"자", 을:"해", 병:"인", 정:"묘", 무:"오", 기:"사", 경:"진·술", 신:"축·미", 임:"신", 계:"유" }); // 일지·시지 적용(둘 다 지원)
...
const MAP_D_JAEGO   = D({ 갑:"진", 을:"진", 병:"축", 정:"축", 무:"축", 기:"축", 경:"미", 신:"미", 임:"술", 계:"술" }); // 길신(재고귀인)
const MAP_D_YANGIN  = D({ 갑:"묘", 을:"진", 병:"오", 정:"미", 무:"오", 기:"미", 경:"유", 신:"술", 임:"자", 계:"축" });
//const MAP_D_GWAEGANG= D({ 경:"진·술", 임:"진·술" }); // 일지 적용 (경/임만)
...
function applyYearToNatal(
  ...
  for (const t of target) {
    for (const p of positions) {
      const m = apply === "ALL" ? findExactPosForBranch(t, natal, p) : findExactPosForBranch(t, natal, p);
...
export function buildShinsalTags({
  ...
  function matchLuckOne(src: Source, luck: string | null) {
    ...
    if (pair && (Lb === pair[0] || Lb === pair[1])) pushLuck(bad, labelLuck_Void(src, voidBasis));

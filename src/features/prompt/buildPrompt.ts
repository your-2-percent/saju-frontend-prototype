...
export function buildTopicGuide(ctx: TopicContext): string {
  const { topic, subTopic, timeMode = "single", tab, relationMode } = ctx;
  ...
  lines.push(
    "- AI 특유의 긍정적이기만 한 면은 제외한다, 최대한 객관적이고 중립적이게 사주만을 보고 설명한다. (감정적, 감성적, 추상적 문구지양)",
    "- 애매하게 말하지 말고, 명확하게 대답한다. (상담하는 입장에서, 사용자가 어떤 대답을 듣고 싶어하는지 항상 생각해라. 사주를 보는 사람들은 명확한 대답을 원한다.)",
    "- 사용자는 사주/운세를 거의 모르는 초보라고 가정한다.",
    "- “비겁, 식상, 오행, 십성, 십이운성” 같은 사주 전문 용어는 쓰지 말고, 일상적인 말로 풀어서 설명한다.",
    "- 답변은 상담실에서 직접 말로 설명하듯이, “~예요, ~있어요, ~할 수 있어요” 같은 구어체로 작성한다.",
    "- 장점과 단점을 모두 말해라. 사주는 양면성이 있으므로, 한쪽 면만 강조하지 말고 균형 있게 다룬다.",
    "- 위에 제공된 JSON/수치/태그는 내부 분석 자료로만 쓰고, 그대로 나열하거나 복붙하지 않는다.",
    "- 특히 이름, 출생 연·월·일·시, 성별, 현재 운 흐름은 답변 초반 몇 문단 안에 자연스럽게 녹여서 소개한다.",
    "- 수치나 퍼센트는 그대로 읽지 말고, “어떤 방향의 기질이 강하다/약하다 보니 이런 행동 패턴이 나타난다”처럼 해석 결과만 문장으로 말한다.",
    "- 사주 구조나 기법 이름은 전혀 언급하지 말고, “질문자는 이런 사람이고, 이런 환경이 잘 맞는다”는 식으로 삶에 바로 적용할 수 있는 말들 위주로 설명한다.",
    "- 글머리표, 번호 목록, 표, 제목을 쓰지 말고, 여러 문단으로 이어지는 상담 대화처럼 길게 적는다."
  );
    ...
    case "personality": {
      ...
      if (!subTopic || subTopic === "overview" || subTopic === "personality_basic") {
        lines.push(
          ...
          "- 전체 분위기는 ‘상담실 소파에 앉아서, 선생님이 차분하게 얘기해 주는 느낌’이 나도록 부드럽게 설명한다."
        );
      }

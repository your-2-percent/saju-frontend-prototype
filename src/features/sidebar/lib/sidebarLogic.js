...
import { UNASSIGNED_LABEL, FOLDER_PRESETS, LS_FOLDERS, LS_FOLDER_ORDER, LS_FOLDER_FAVS, LS_DISABLED_PRESETS, displayFolderLabel, } from "@/features/sidebar/model/folderModel";
...
export function useSidebarLogic(list, update) {
    // ===== 로컬스토리지 상태들
    const [customFolders, setCustomFolders] = useLocalStorageState(LS_FOLDERS, []);
    const [folderFavMap, setFolderFavMap] = useLocalStorageState(LS_FOLDER_FAVS, {});
    const [folderOrder, setFolderOrder] = useLocalStorageState(LS_FOLDER_ORDER, []);
    const [disabledPresets, setDisabledPresets] = useLocalStorageState(LS_DISABLED_PRESETS, []);
    // 임시 UI 상태
    const [folderOpenMap, setFolderOpenMap] = useLocalStorageState("ms_folder_open", {});
    const [memoOpenMap, setMemoOpenMap] = useLocalStorageState("ms_memo_open", {});
    const [newFolderName, setNewFolderName] = useLocalStorageState("ms_new_folder_tmp", "");
    // Confirm 쓰로틀
    const lastConfirmAtRef = useRef(0);
    function confirmThrottled(message, onOk) {
        const now = Date.now();
        if (now - lastConfirmAtRef.current < 450)
            return;
    ...
    const presetsEffective = useMemo(() => FOLDER_PRESETS.filter((f) => !disabledPresets.includes(f)), [disabledPresets]);
    // 폴더 목록(UNASSIGNED 제외)
    const allFoldersBase = useMemo(() => [...presetsEffective, ...customFolders], [presetsEffective, customFolders]);
    // ===== 폴더 순서 정합성 유지
    useEffect(() => {
        setFolderOrder((prev) => {
    ...
    const { grouped, unassignedItems } = useMemo(() => {
        ...
        const byFav = (a, b) => Number(!!b.favorite) - Number(!!a.favorite);
        for (const k of Object.keys(g))
            g[k] = g[k].slice().sort(byFav);
        outside.sort(byFav);

// components/sidebarUtils.ts
import type { MyeongSik } from "@/shared/lib/storage";
import { getCorrectedDate } from "@/shared/lib/core/timeCorrection";

/** 폴더명 유니크 생성 */
export function uniqueFolderName(base: string, existingList: string[]) {
...
export function fmtBirthKR(yyyymmdd: string, hhmm?: string) {
  if (!/^\d{8}$/.test(yyyymmdd)) return yyyymmdd || "";
  const y = yyyymmdd.slice(0, 4);
  const m = yyyymmdd.slice(4, 6);
  const d = yyyymmdd.slice(6, 8);
  if (hhmm && /^\d{4}$/.test(hhmm)) return `${y}년 ${m}월 ${d}일생 ${hhmm.slice(0, 2)}:${hhmm.slice(2, 4)}분`;
  if (hhmm === "모름") return `${y}년 ${m}월 ${d}일생 (시간 모름)`;
  return `${y}년 ${m}월 ${d}일생`;
}

export function formatPlaceDisplay(name?: string) {
  if (!name) return "";
...
export function fmtLatLon(m: MyeongSik) {
...
export function calcAgeFromBirthDay(yyyymmdd: string) {
...
export function getCorrectedLocal(m: MyeongSik) {
  if (m.correctedLocal) return m.correctedLocal;
  if (!m.birthDay) return "";
  if (m.birthTime === "모름" || !m.birthTime) return "";
...
export function getGanjiString(m: MyeongSik) {
  return (m.ganji ?? "")
    .replace(/\r\n/g, "\n")

...
export function displayFolderLabel(v) {
    if (!v || v === "미분류")
        return UNASSIGNED_LABEL;
    return v;
}
/** 저장용: "폴더지정하지않음"/"미분류"/빈 값 → undefined */
export function normalizeFolderValue(v) {
    if (!v || v === UNASSIGNED_LABEL || v === "미분류")
        return undefined;
    return v;
}
/** 입력 폼(새 명식) 드롭다운 옵션: 등록된(프리셋-숨김 제외 + 커스텀)만 */
export function getFolderOptionsForInputNow() {
    try {
        ...
        const disabled = rawDisabled ? JSON.parse(rawDisabled) : [];
